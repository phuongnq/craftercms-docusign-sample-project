...
public final class EnvelopeHelpers {
    ...
    static Template loadHtmlTemplateFromSite(SiteItemService siteItemService, String path) throws IOException {
        Configuration cfg = new Configuration(Configuration.VERSION_2_3_29)
        cfg.setLocale(Locale.US)
        cfg.setDefaultEncoding(StandardCharsets.UTF_8.name())
        def is = CrafterCMSContentHelpers.getInputStream(siteItemService, path)
        String source = StreamUtils.copyToString(is, StandardCharsets.UTF_8)
        return new Template(path, source, cfg)
    }
    ...
    public static String createHtmlFromTemplateAsset(SiteItemService siteItemService, String path, String objectName, Object value) throws IOException {
        Template template = loadHtmlTemplateFromSite(siteItemService, path)
        return createHtmlFromTemplate(template, objectName, value)
    }
    ...
    /**
     * Loads document from a CrafterCMS Asset location
     * @param siteItemService A SiteItemService instance
     * @param fileName name of the file to load document the extension of the
     * loading file determines an extension of the created document
     * @param docName the name of the document it may be differ from the file
     * @param docId identifier of the created document
     ...
     * @throws IOException if document cannot be loaded due to some reason
     */
    public static Document createDocumentFromAsset(SiteItemService siteItemService, String fileName, String docName, String docId) throws IOException {
        byte[] buffer = CrafterCMSContentHelpers.getContentBytes(siteItemService, fileName)
        String extention = FilenameUtils.getExtension(fileName)
        return createDocument(buffer, docName, extention, docId)
    }
